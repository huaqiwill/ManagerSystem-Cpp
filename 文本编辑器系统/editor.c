...
int test(Hnode *Hhead, int n)
{
       ...
       if (tail->ch != 13) /*若此行行尾不是回车键*/
       {
              ...
              else /*若此行行尾和行中都没有回车键,或者q->nextl不为空*/
              {
                     ...
                     q->next = p2;         /**/
                     if (q != NULL)
                            test(Hhead, ++n); /*若行单链表第n个节点后还有节点，继续test()的相同处理*/
              }
       }
...
void control(int A, Hnode *Hhead)
{
       ...
       if (A == CR && flag == 1) /*ctrl+→，光标已经跳至新处，将当前光标所在位置的字符的坐标和值保存在r数组中*/
       {
              ...
              if (r[abs(value)].ch == -1)
                     r[abs(value)].ch = 13; /*若第line行，第col列的字符为回车键，则返回-1*/
              value--;
       }
...
int menuctrl(Hnode *Hhead, int A) /*菜单控制*/
{
       ...
       if (A == F1)
       {
              drawmenu(0, flag);
              value = 300;
       } /*显示File及其子菜单,并将光带显示在第一个子菜单上*/
       if (A == F2)
       {
              drawmenu(1, flag);
              value = 301;
       } /*显示Edit及其子菜单,并将光带显示在第一个子菜单上*/
       if (A == F3)
       {
              drawmenu(2, flag);
              value = 302;
       } /*显示Help及其子菜单,并将光带显示在第一个子菜单上*/
       ...
       if (A == F1 || A == F2 || A == F3)
       {
              while ((B = bioskey(0)) != ESC) /*选择用户按键*/
              {
                     if (flag == 0)
                            flag = 100;
                     if (value == 0)
                            value = 300; /*此value为局部变量*/

                     if (B == UP)
                            drawmenu(value, --flag); /*循环上下移*/
                     if (B == DOWN)
                            drawmenu(value, ++flag); /*循环上下移*/
...
void opens(Hnode *Hp)
{
       ...
       while (!feof(fp))
       {
              ...
              if (temp == 10)
              { /*若为换行符，将其转换为回车符，因为在程序中，是按回车符处理的*/
...
void main()
{
       ...
       while (1)
       {
              ...
              if ((A == F1) || (A == F2) || (A == F3) || (a < 32 || a > 127))
              {
                     ...
                     {
                            clrscr();
                            cprintf("<Help> F1:File  F2:Edit F3:Help ");
                            getch();
                            gotoxy(3, 3);
                            view(Hhead);
                     }
                     ...
                     {
                            clrscr();
                            cprintf("Abort:Version 2.0 Tel:XXXXXXXXXX");
                            getch();
                            gotoxy(3, 3);
                            view(Hhead);
                     }
